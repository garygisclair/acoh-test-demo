...
var createTamagui = process.env.NODE_ENV !== "development" ? createTamaguiCore : (conf) => {
  const sizeTokenKeys = ["$true"], hasKeys = /* @__PURE__ */ __name((expectedKeys, obj) => expectedKeys.every((k) => typeof obj[k] < "u"), "hasKeys"), tamaguiConfig2 = createTamaguiCore(conf);
  ...
  addChildThemes(childThemeDefinition, options) {
    ...
    const currentThemeNames = Object.keys(currentThemes), incomingThemeNames = Object.keys(childThemeDefinition), namesWithDefinitions = currentThemeNames.flatMap((prefix) => {
      ...
      return avoidNestingWithin && avoidNestingWithin.some((avoidName) => prefix.startsWith(avoidName) || prefix.endsWith(avoidName)) ? [] : incomingThemeNames.map((subName) => {
        ...
        return "avoidNestingWithin" in definition && definition.avoidNestingWithin.some((name) => (name === "light" || name === "dark") && prefix.includes("_") ? false : prefix.startsWith(name) || prefix.endsWith(name)) || prefix.endsWith(`_${subName}`) || fullName in currentThemes ? null : [fullName, definition];
      }).filter(Boolean);
...
import { createFont as createFont2, getVariableValue as getVariableValue2, isWeb as isWeb3 } from "@tamagui/core";
var createInterFont = /* @__PURE__ */ __name((font = {}, {
  ...
  return createFont2({
    family: isWeb3 ? 'Inter, -apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif' : "Inter",
...
import { createFont as createFont3, isWeb as isWeb4 } from "@tamagui/core";
var createSilkscreenFont = /* @__PURE__ */ __name((font = {}) => createFont3({
  family: isWeb4 ? "Silkscreen, Fira Code, Monaco, Consolas, Ubuntu Mono, monospace" : "Silkscreen",
  size: size2,
